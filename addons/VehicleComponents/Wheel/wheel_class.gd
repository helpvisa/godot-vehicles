...
func updateWheelPosition(parent):
	...
	var query = PhysicsRayQueryParameters3D.create(parent.to_global(offset + (Vector3.UP * radius) + (Vector3.UP * suspensionRange)),\
		parent.to_global(offset + (Vector3.DOWN * radius) + (Vector3.DOWN * suspensionRange)),\
		colMask)
